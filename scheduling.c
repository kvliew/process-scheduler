...
void step(struct cpu *processor, int *processesCompleted) {
    if(processor->state == 0 && (processor->front != -1) &&
    (processor->back != -1)) { // CPU is idle and its waiting queue is not empty
        ...
        // find shortest process in cpuQueue in each time step
        ...
        if(processor->cpuRemainingTime > 1) { // CPU is still running a process
            ...
            processor->currentlyRunning.processId, *processesCompleted);
            processor->state = 0;
            (*processesCompleted)++;
        }
    }
...
void printRunning(struct process processEntry, int id) {
    ...
    currentlyRunning.processId, currentlyRunning.executionTime, id);
}
